...
namespace Yandex.Music.Api.Common.Ynison
{
    public class YnisonPlayer : IDisposable
    {
        ...
        {
            YYnisonVersion version = new YYnisonVersion() {
                DeviceId = storage.DeviceId,
                Version = "0"
            };

            YYnisonUpdateFullStateMessage fullState = new YYnisonUpdateFullStateMessage() {
                UpdateFullState = new YYnisonFullState() {
                    Device = new YYnisonDevice() {
                        Capabilities = new YYnisonDeviceCapabilities() {
                            CanBePlayer = true
                        },
                    ...
                    PlayerState = new YYnisonPlayerState() {
                        PlayerQueue = new YYnisonPlayerQueue() {
                            Version = version
                        },
                        Status = new YYnisonPlayerStateStatus() {
                            Version = version
                        }
                    }
                }
        ...
        {
            YYnisonUpdatePlayerStateMessage update = new YYnisonUpdatePlayerStateMessage() {
                UpdatePlayerState = State.PlayerState
            };

            update.UpdatePlayerState.Status.Version = new YYnisonVersion() {
                DeviceId = storage.DeviceId
            };

            update.UpdatePlayerState.PlayerQueue.Version = new YYnisonVersion() {
                DeviceId = storage.DeviceId
            };
        ...
        */
        ...
        internal YnisonPlayer(YandexMusicApi api, AuthStorage authStorage)
        {
            ...
            redirector = new YnisonWebSocket();
            state = new YnisonWebSocket();
        }
