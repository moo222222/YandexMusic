...
namespace Yandex.Music.Api.Common.Ynison
{
    public class YnisonWebSocket: IDisposable
    {
        ...
        private readonly ClientWebSocket socketClient = new ClientWebSocket();
        ...
        private string GetProtocolData(string deviceId, string redirectTicket)
        {
            ...
            Dictionary<string, string> protocol = new Dictionary<string, string>() {
        ...
        public /*Value*/Task Send(string json)
        {
            ArraySegment<byte> message = new ArraySegment<byte>(Encoding.UTF8.GetBytes(json));
